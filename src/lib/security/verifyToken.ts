...
export default async (type: 'access' | 'refresh' | 'signIn', token: string): Promise<TokenPayload> => {
  switch (type) {
    case 'access': return await verifyToken(token, await crypto.subtle.importKey('jwk', JSON.parse(await env.get('JWK_FOR_ACCESS_TOKEN_PUBLIC')), getAlgorithmOptions('import'), true, ['verify']))
    case 'refresh': return await verifyToken(token, await crypto.subtle.importKey('jwk', JSON.parse(await env.get('JWK_FOR_REFRESH_TOKEN_PUBLIC')), getAlgorithmOptions('import'), true, ['verify']))
    case 'signIn': return await verifyToken(token, await crypto.subtle.importKey('jwk', JSON.parse(await env.get('JWK_FOR_SIGN_IN_PUBLIC')), getAlgorithmOptions('import'), true, ['verify']))
  }
}
